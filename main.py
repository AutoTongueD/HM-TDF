...
def train(net, train_img_name, train_features, train_labels, 
    ...
    # Determine the number of hard examples to select for each class
...
def get_k_fold_data(k, i, X, Xf, y): # Get training and validation data for k-fold cross-validation.
    assert k > 1
    fold_size = len(X) // k
